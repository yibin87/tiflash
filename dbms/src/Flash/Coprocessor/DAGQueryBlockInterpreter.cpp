...
void DAGQueryBlockInterpreter::handleJoin(const tipb::Join & join, DAGPipeline & pipeline, SubqueryForSet & right_query, size_t fine_grained_shuffle_count)
{
    ...
    JoinPtr join_ptr = std::make_shared<Join>(
        ...
        log->identifier(),
	enableFineGrainedShuffle(fine_grained_shuffle_count) && settings.enable_fine_grained_join,
    ...
    build_pipeline.transform([&](auto & stream) {
        stream = std::make_shared<HashJoinBuildBlockInputStream>(stream, join_ptr, get_concurrency_build_index(), log->identifier(), 100000);
        stream->setExtraInfo(
