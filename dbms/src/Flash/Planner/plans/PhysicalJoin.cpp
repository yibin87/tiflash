    ...
    const FineGrainedShuffle & fine_grained_shuffle_)
{
    ...
    JoinPtr join_ptr = std::make_shared<Join>(
        ...
        log->identifier(),
        (fine_grained_shuffle_.enable() && context.getSettingsRef().fine_grained_join_mode > 0),
...
void PhysicalJoin::probeSideTransform(DAGPipeline & probe_pipeline, Context & context, size_t max_streams)
{
    ...
    join_probe_actions->add(ExpressionAction::ordinaryJoin(0, join_ptr, columns_added_by_join));
